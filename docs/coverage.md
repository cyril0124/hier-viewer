...
The generator copies regular `.xml` and `.html` files into a signature-named `coverage-*` directory inside the output bundle and adds the manifest attribute automatically. The input report is read-only, symlinks are not copied, and output cannot be inside the input report. An unchanged report and root selection reuse the existing directory without copying its files again. Older report copies remain valid when a report changes; remove stale `coverage-*` directories after confirming that no deployed page uses them. Keep the entire output directory when deploying; runtime access to the original report and the local import API is unnecessary. Omitting the coverage options on a later generation removes automatic loading from the new page, but does not delete earlier copies from the output directory.
