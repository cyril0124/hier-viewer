...
点击视图切换栏中的 **Coverage**，在[三栏工作区](docs/coverage.md#coverage-workspace)查看实例覆盖率、模块源码和指标明细。点击实例或指标单元格即可查看。拖动分隔栏调整宽度；窄屏通过 Instances、Source、Details 页签切换。

工具栏 Coverage 旁的 X 关闭着色，重新选择指标即可恢复。
