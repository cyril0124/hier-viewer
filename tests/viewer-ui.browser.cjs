...
(async () => {
  ...
  try {
    ...
    // Zoom painting and its status update are coalesced into the next animation frame.
    await page.waitForFunction(() => document.querySelector('#status-left').textContent.includes('zoom 1.25x'));
    ...
    await page.mouse.move(treemapBox.x + treemapBox.width * 0.6 + 40, treemapBox.y + treemapBox.height * 0.6 + 20, { steps: 8 });
    await page.mouse.up();
    assert.notDeepEqual(await treemapCanvas.screenshot(), zoomed, 'Treemap pan did not change pixels');
    await page.locator('#fit-btn').click();
    assert.ok((await page.locator('#status-left').textContent()).includes('zoom 1.00x'), 'Treemap Fit did not reset zoom');
    ...
    await shot('source');
    await page.locator('.source-lineno').first().click();
    await page.locator('.source-bookmark-card').waitFor();
    await page.locator('.source-bookmark-action:not(.remove)').click();
    await page.locator('.source-bookmark-input').fill('migration-bookmark');
    await page.locator('.source-bookmark-input').press('Enter');
    assert.equal(await page.locator('.source-bookmark-title').textContent(), 'migration-bookmark');
    await page.reload();
    await page.locator('#loading-overlay').waitFor({ state: 'hidden' });
    if (!(await page.locator('#tree-panel').isVisible())) await page.locator('#toggle-tree-btn').click();
    await page.locator('.tree-row:has(.tree-indicator.leaf) .tree-entry').first().click();
    await page.locator('.source-bookmark-card').waitFor();
    assert.equal(await page.locator('.source-bookmark-title').textContent(), 'migration-bookmark', 'Bookmark name did not survive reload');
    await page.locator('.source-bookmark-action.remove').click();
    assert.equal(await page.locator('.source-bookmark-card').count(), 0);
